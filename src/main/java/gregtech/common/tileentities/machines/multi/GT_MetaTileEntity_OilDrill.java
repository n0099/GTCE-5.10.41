...
public class GT_MetaTileEntity_OilDrill extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public boolean checkRecipe(ItemStack aStack) {
        ...
        if (getBaseMetaTileEntity().getBlockOffset(
                EnumFacing.field_82609_l[getBaseMetaTileEntity().getBackFacing()].func_82601_c(),
                getYOfPumpHead() - 1 - getBaseMetaTileEntity().getYCoord(),
                EnumFacing.field_82609_l[getBaseMetaTileEntity().getBackFacing()].func_82599_e()) != Blocks.field_150357_h) {
            if (completedCycle) {
                moveOneDown();
            }
            tFluid = null;
