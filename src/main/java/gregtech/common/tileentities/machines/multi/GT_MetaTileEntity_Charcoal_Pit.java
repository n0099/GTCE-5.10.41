...
public class GT_MetaTileEntity_Charcoal_Pit extends GT_MetaTileEntity_MultiBlockBase {
    ...
    private boolean checkRecursiveBlocks() {
        ...
        while (!tList2.isEmpty()) {
            ...
            tList2.remove(0);
            if (!checkAllBlockSides(tPos.func_177958_n(), tPos.func_177956_o(), tPos.func_177952_p(), tList1, tList2)) {
                return false;
            }
        }
        if (running) {
            for (BlockPos tPos : tList1) {
                if (isWoodLog(this.getBaseMetaTileEntity().getBlockStateOffset(tPos.func_177958_n(), tPos.func_177956_o(), tPos.func_177952_p())))
                    this.getBaseMetaTileEntity().getWorldObj().func_180501_a(
                            getBaseMetaTileEntity().getWorldPos().func_177982_a(tPos.func_177958_n(), tPos.func_177956_o(), tPos.func_177952_p()),
                            GregTech_API.sBlockReinforced.func_176203_a(4), 3);
            }
            running = false;
            return false;
        } else {
    ...
    private boolean checkAllBlockSides(int aX, int aY, int aZ, ArrayList<BlockPos> aList1, ArrayList<BlockPos> aList2) {
        ...
        if (aY + 1 < 1 && (isWoodLog(tBlock))) {
            if (!aList1.contains(new BlockPos(aX, aY + 1, aZ)) && (!aList2.contains(new BlockPos(aX, aY + 1, aZ))))
                p3 = true;
        } else if (!(isDirt(tBlock) || (aX == 0 && aY == -1 && aZ == 0 && tBlock.func_177230_c() == GregTech_API.sBlockMachines))) {
            return false;
        }
        ...
        if (aY - 1 > -6 && (isWoodLog(tBlock))) {
            if (!aList1.contains(new BlockPos(aX, aY - 1, aZ)) && (!aList2.contains(new BlockPos(aX, aY - 1, aZ))))
                p4 = true;
        } else if (tBlock.func_177230_c() != Blocks.field_150336_V) {
            return false;
        }
