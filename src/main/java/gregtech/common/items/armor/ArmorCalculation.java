	...
	public static float[] calculateArmor(ItemStack[] parts) {
		...
		if (parts != null) {
			...
			for (ItemStack part : parts) {
				if (part != null) {
					ItemData data = GT_OreDictUnificator.getItemData(part);
					...
					} else if (GT_ModHandler.isChargerItem(part)) {
						def[12] = def[12] + (float) ElectricItem.manager.getCharge(part);
						def[0] = (float) (def[0] + Math.pow(ElectricItem.manager.getCharge(part), 0.33f));
					}
					//else if (Loader.isModLoaded("Thaumcraft") && parts[i].getItem() instanceof IRevealer) {
					//	def[13] = 1;
					//	def[31] += 100;
					//}
					else if (part.getItem().getUnlocalizedName().equals("ic2.itemNightvisionGoggles")) {
						def[14] = 1;
						def[31] += 100;
					} else if (part.getItem().getUnlocalizedName().equals("gt.meta.spring")) {// Once readded: GT Motors
					...
					} else if (part.getItem().getUnlocalizedName().equals("gt.meta.springSmall")) {// Once Readded: GT Electric Pistons
						switch (part.getItem().getDamage(part)) {
							case 8630:
								def[18] += 3;
								def[19] += 200;
					...
					} else if (part.getItem().getUnlocalizedName().equals("gt.meta.Electrolyzer")) {// Once Readded: GT Electrolyzer
						switch (part.getItem().getDamage(part)) {
							case 8630:
					...
					} else if (part.getItem().getUnlocalizedName().equals("gte.meta.jetpack")) {// jeptack parts
						switch (part.getItem().getDamage(part)) {
							case 0:
