...
public class GT_NeutronReflector_Item
    ...
    public boolean acceptUraniumPulse(ItemStack yourStack, IReactor reactor, ItemStack pulsingStack, int youX, int youY, int pulseX, int pulseY, boolean heatrun) {
        if (!heatrun) {
            if (!GregTech_API.mIC2Classic&&(pulsingStack.func_77973_b() instanceof ic2.core.item.reactor.ItemReactorMOX)) {
                ...
                float tEnergy = 1.0f;
                if (pulsingStack.func_77973_b() instanceof GT_RadioactiveCellIC_Item) {
                    tEnergy = (float) ((GT_RadioactiveCellIC_Item) pulsingStack.func_77973_b()).sEnergy;
                }
