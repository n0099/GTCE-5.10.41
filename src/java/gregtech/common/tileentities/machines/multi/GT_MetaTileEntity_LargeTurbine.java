    ...
    public boolean checkRecipe(ItemStack aStack) {
        ...
        if (tFluids.size() > 0) {
            if (baseEff == 0 || optFlow == 0 || counter >= 1000 || this.getBaseMetaTileEntity().hasWorkJustBeenEnabled()
                ...
                counter = 0;
                baseEff = (int) ((50.0F
                        + (10.0F * ((GT_MetaGenerated_Tool) aStack.func_77973_b()).getToolCombatDamage(aStack))) * 100);
                optFlow = (int) Math.max(Float.MIN_NORMAL,
                        ((GT_MetaGenerated_Tool) aStack.func_77973_b()).getToolStats(aStack).getSpeedMultiplier()
                                * ((GT_MetaGenerated_Tool) aStack.func_77973_b()).getPrimaryMaterial(aStack).mToolSpeed
                                * 50);
            } else {
                counter++;
            }
...
//	            this.mEfficiencyIncrease = (-10);
    ...
    public String[] getInfoData() {
        ...
        if (mInventory[1] != null && mInventory[1].func_77973_b() instanceof GT_MetaGenerated_Tool_01) {
