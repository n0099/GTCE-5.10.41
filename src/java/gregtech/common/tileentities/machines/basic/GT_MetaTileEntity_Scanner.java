...
public class GT_MetaTileEntity_Scanner
        ...
        if (getOutputAt(0) != null) {
            ...
            if (ItemList.Tool_DataOrb.isStackEqual(getSpecialSlot(), false, true)) {
                ...
                if ((tData != null) && ((tData.mPrefix == OrePrefixes.dust) || (tData.mPrefix == OrePrefixes.cell)) && (tData.mMaterial.mMaterial.mElement != null) && (!tData.mMaterial.mMaterial.mElement.mIsIsotope) && (tData.mMaterial.mMaterial != Materials.Magic) && (tData.mMaterial.mMaterial.getMass() > 0L)) {
                    getSpecialSlot().field_77994_a -= 1;
                    aStack.field_77994_a -= 1;
            ...
            if (ItemList.Tool_DataStick.isStackEqual(getSpecialSlot(), false, true)) {
                ...
                if (aStack.func_77973_b() == Items.field_151098_aY) {
                    ...
                    this.mOutputItems[0] = GT_Utility.copyAmount(1L, new Object[]{getSpecialSlot()});
                    this.mOutputItems[0].func_77982_d(GT_Utility.getNBTContainingShort(new NBTTagCompound(), "map_id", (short) aStack.func_77952_i()));
                    this.mMaxProgresstime = (128 / (1 << this.mTier - 1));
                    this.mEUt = (32 * (1 << this.mTier - 1) * (1 << this.mTier - 1));
                    return 2;
                }
            ...
            if(ItemList.Tool_DataStick.isStackEqual(getSpecialSlot(), false, true)&& aStack !=null){
                ...
                tNBT.func_74782_a("output", tRecipe.mOutput.func_77955_b(new NBTTagCompound()));
                tNBT.func_74768_a("time", tRecipe.mDuration);
                tNBT.func_74768_a("eu", tRecipe.mEUt);
                for(int i = 0 ; i < tRecipe.mInputs.length ; i++){
                	
                	tNBT.func_74782_a(""+i, tRecipe.mInputs[i].func_77955_b(new NBTTagCompound()));
                }
                for(int i = 0 ; i < tRecipe.mFluidInputs.length ; i++){
                	
                	tNBT.func_74782_a("f"+i, tRecipe.mFluidInputs[i].writeToNBT(new NBTTagCompound()));
                }
                tNBT.func_74778_a("author", "Assembly Line Recipe Generator");
                NBTTagList tNBTList = new NBTTagList();
                tNBTList.func_74742_a(new NBTTagString("Constructionplan for "+tRecipe.mOutput.field_77994_a+" "+GT_LanguageManager.getTranslation(tRecipe.mOutput.func_82833_r())+". Needed EU/t: "+tRecipe.mEUt+" Productiontime: "+(tRecipe.mDuration/20)));
                for(int i=0;i<tRecipe.mInputs.length;i++){
                	if(tRecipe.mInputs[i]!=null){
                		tNBTList.func_74742_a(new NBTTagString("Input Bus "+(i+1)+": "+tRecipe.mInputs[i].field_77994_a+" "+GT_LanguageManager.getTranslation(tRecipe.mInputs[i].func_82833_r())));
                	}
                }
                for(int i=0;i<tRecipe.mFluidInputs.length;i++){
                	if(tRecipe.mFluidInputs[i]!=null){
                		tNBTList.func_74742_a(new NBTTagString("Input Hatch "+(i+1)+": "+tRecipe.mFluidInputs[i].amount+"L "+GT_LanguageManager.getTranslation(tRecipe.mFluidInputs[i].getLocalizedName())));
                	}
                }
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        if (mProgresstime >= (mMaxProgresstime - 1)) {
            if ((this.mOutputItems[0] != null) && (this.mOutputItems[0].func_77977_a().equals("gt.metaitem.01.32707"))) {
                GT_Mod.instance.achievements.issueAchievement(aBaseMetaTileEntity.getWorldObj().func_72924_a(aBaseMetaTileEntity.getOwnerName()), "scanning");
            }
        }
