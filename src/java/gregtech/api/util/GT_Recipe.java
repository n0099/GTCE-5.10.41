...
public class GT_Recipe {
    ...
    protected GT_Recipe(boolean aOptimize, ItemStack[] aInputs, ItemStack[] aOutputs, Object aSpecialItems, int[] aChances, FluidStack[] aFluidInputs, FluidStack[] aFluidOutputs, int aDuration, int aEUt, int aSpecialValue) {
        ...
        for (ItemStack aInput : aInputs)
            if (aInput != null && Items.field_151008_G.getDamage(aInput) != W)
            ...
            for (byte i = (byte) Math.min(64, aDuration / 16); i > 1; i--)
                if (aDuration / i >= 16) {
                    ...
                    for (int j = 0, k = tList.size(); temp && j < k; j++)
                        if (tList.get(j).field_77994_a % i != 0) temp = false;
                    ...
                    if (temp) {
                        for (int j = 0, k = tList.size(); j < k; j++) tList.get(j).field_77994_a /= i;
...
//		checkCellBalance();
    ...
    public GT_Recipe(int aEUt, int aDuration, ItemStack aInput1, ItemStack aOutput1) {
        this(true, new ItemStack[]{aInput1, ItemList.Circuit_Integrated.getWithDamage(0, aInput1.field_77994_a)}, new ItemStack[]{aOutput1}, null, null, null, null, Math.max(aDuration, 1), Math.max(aEUt, 1), 0);
        if (mInputs.length > 0 && mOutputs[0] != null) {
    ...
    public static class GT_Recipe_Map_Microwave extends GT_Recipe_Map_NonGTRecipes {
        ...
        public GT_Recipe findRecipe(IHasWorldObjectAndCoords aTileEntity, GT_Recipe aRecipe, boolean aNotUnificated, long aVoltage, FluidStack[] aFluids, ItemStack aSpecialSlot, ItemStack... aInputs) {
            ...
            if (GT_Utility.areStacksEqual(aInputs[0], new ItemStack(Items.field_151122_aG, 1, W))) {
            ...
            for (ItemStack tStack : new ItemStack[]{GT_Utility.getContainerItem(aInputs[0], true), aInputs[0], GT_Utility.getContainerItem(tOutput, true), tOutput})
                if (tStack != null) {
                    if (GT_Utility.areStacksEqual(tStack, new ItemStack(Blocks.field_150424_aL, 1, W), true)
                            || GT_Utility.areStacksEqual(tStack, new ItemStack(Blocks.field_150335_W, 1, W), true)
                            || GT_Utility.areStacksEqual(tStack, new ItemStack(Items.field_151110_aK, 1, W), true)
                            || GT_Utility.areStacksEqual(tStack, new ItemStack(Items.field_151154_bQ, 1, W), true)
                            || GT_Utility.areStacksEqual(tStack, new ItemStack(Items.field_151152_bP, 1, W), true)
                            || GT_Utility.areStacksEqual(tStack, new ItemStack(Items.field_151059_bz, 1, W), true)
                            ) {
                    ...
                    if (TileEntityFurnace.func_145952_a(tStack) > 0) {
        ...
        public GT_Recipe findRecipe(IHasWorldObjectAndCoords aTileEntity, GT_Recipe aRecipe, boolean aNotUnificated, long aVoltage, FluidStack[] aFluids, ItemStack aSpecialSlot, ItemStack... aInputs) {
            ...
            for (ItemStack aMold : aInputs) {
                if (ItemList.Shape_Mold_Credit.isStackEqual(aMold, false, true)) {
                    ...
                    if (!tNBT.func_74764_b("credit_security_id")) tNBT.func_74772_a("credit_security_id", System.nanoTime());
    ...
    public static class GT_Recipe_Map_Printer extends GT_Recipe_Map {
        ...
        public GT_Recipe findRecipe(IHasWorldObjectAndCoords aTileEntity, GT_Recipe aRecipe, boolean aNotUnificated, long aVoltage, FluidStack[] aFluids, ItemStack aSpecialSlot, ItemStack... aInputs) {
            ...
            if (rRecipe == null) {
                ...
                if (aInputs[0].func_77973_b() == Items.field_151164_bB) {
                ...
                if (aInputs[0].func_77973_b() == Items.field_151098_aY) {
                    if (!ItemList.Tool_DataStick.isStackEqual(aSpecialSlot, false, true)) return null;
                    NBTTagCompound tNBT = aSpecialSlot.func_77978_p();
                    if (tNBT == null || !tNBT.func_74764_b("map_id")) return null;

                    rRecipe = rRecipe.copy();
                    rRecipe.mCanBeBuffered = false;
                    rRecipe.mOutputs[0].func_77964_b(tNBT.func_74765_d("map_id"));
                    return rRecipe;
                }
                if (ItemList.Paper_Punch_Card_Empty.isStackEqual(aInputs[0], false, true)) {
                    if (!ItemList.Tool_DataStick.isStackEqual(aSpecialSlot, false, true)) return null;
                    NBTTagCompound tNBT = aSpecialSlot.func_77978_p();
                    if (tNBT == null || !tNBT.func_74764_b("GT.PunchCardData")) return null;
