            ...
            for (byte i = 0, j = 0; i < 6 && aAmperage > rUsedAmperes; i++)
                if (aEmitter.outputsEnergyTo(i)) {
                    ...
                    if (tTileEntity instanceof IEnergyConnected) {
                        ...
                        if (((IEnergySink) tTileEntity).acceptsEnergyFrom(aEmitter, EnumFacing.field_82609_l[j])) {
                            while (aAmperage > rUsedAmperes && ((IEnergySink) tTileEntity).getDemandedEnergy() > 0 && ((IEnergySink) tTileEntity).injectEnergy(EnumFacing.field_82609_l[j], aVoltage, aVoltage) < aVoltage)
                                rUsedAmperes++;
                        }
                    } else if (tTileEntity instanceof IEnergyReceiver) {
                        EnumFacing tDirection = EnumFacing.field_82609_l[i].func_176734_d();
                        ...
                        if (GregTech_API.mRFExplosions && GregTech_API.sMachineExplosions && ((IEnergyReceiver) tTileEntity).getMaxEnergyStored(tDirection) < rfOut * 600) {
                            if (rfOut > 32 * GregTech_API.mEUtoRF / 100) {
                                ...
                                BlockPos position = tTileEntity.func_174877_v();
                                World tWorld = tTileEntity.func_145831_w();
                                GT_Utility.sendSoundToPlayers(tWorld, GregTech_API.sSoundList.get(209), 1.0F, -1, position.func_177958_n(), position.func_177956_o(), position.func_177952_p());
                                tWorld.func_175698_g(position);
                                if (GregTech_API.sMachineExplosions)
                                    tWorld.func_72876_a(null, position.func_177958_n() + 0.5, position.func_177956_o() + 0.5, position.func_177952_p() + 0.5, tStrength, true);
                            }
                        }
                    }
                }
