...
public abstract class GT_MetaBase_Item extends GT_Generic_Item implements ISpecialElectricItem, IElectricItemManager, IFluidContainerItem {
    ...
    public boolean onLeftClickEntity(ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        ...
        if (tList != null) {
            for (IItemBehaviour<GT_MetaBase_Item> tBehavior : tList) {
                if (tBehavior.onLeftClickEntity(this, aStack, aPlayer, aEntity, aPlayer.func_184600_cs())) {
                    if (aStack.field_77994_a <= 0) aPlayer.func_184201_a(EntityEquipmentSlot.MAINHAND, null);
                    return true;
                }
            }
        }
    ...
    @Override
    public EnumActionResult func_180614_a(ItemStack aStack, EntityPlayer aPlayer, World aWorld, BlockPos blockPos, EnumHand hand, EnumFacing aSide, float hitX, float hitY, float hitZ) {
        use(aStack, 0, aPlayer);
        isItemStackUsable(aStack);
        ArrayList<IItemBehaviour<GT_MetaBase_Item>> tList = mItemBehaviors.get((short) getDamage(aStack));
        if (tList != null) {
            for (IItemBehaviour<GT_MetaBase_Item> tBehavior : tList) {
                if (tBehavior.onItemUse(this, aStack, aPlayer, aWorld, blockPos, aSide, hitX, hitY, hitZ, hand)) {
                    if (aStack.field_77994_a <= 0) aPlayer.func_184611_a(hand, null);
    ...
    @Override
    public final void func_77624_a(ItemStack aStack, EntityPlayer aPlayer, List<String> aList, boolean aF3_H) {
        String tKey = func_77667_c(aStack) + ".tooltip", tString = GT_LanguageManager.getTranslation(tKey);
    ...
    public final boolean canProvideEnergy(ItemStack aStack) {
        Long[] tStats = getElectricStats(aStack);
        return tStats != null && (tStats[3] > 0 || (aStack.field_77994_a == 1 && (tStats[3] == -2 || tStats[3] == -3)));
    }
    ...
    public final double charge(ItemStack aStack, double aCharge, int aTier, boolean aIgnoreTransferLimit, boolean aSimulate) {
        Long[] tStats = getElectricStats(aStack);
        if (tStats == null || tStats[2] > aTier || !(tStats[3] == -1 || tStats[3] == -3 || (tStats[3] < 0 && aCharge == Integer.MAX_VALUE)) || aStack.field_77994_a != 1)
            return 0;
    ...
    public final double discharge(ItemStack aStack, double aCharge, int aTier, boolean aIgnoreTransferLimit, boolean aBatteryAlike, boolean aSimulate) {
        ...
        if (tStats[3] > 0) {
            if (aCharge < tStats[3] || aStack.field_77994_a < 1) return 0;
            if (!aSimulate) aStack.field_77994_a--;
            return tStats[3];
        }
    ...
    public final boolean use(ItemStack aStack, double aAmount, EntityLivingBase aPlayer) {
        ...
        if (aPlayer instanceof EntityPlayer && ((EntityPlayer) aPlayer).field_71075_bZ.field_75098_d) return true;
    ...
    public final void chargeFromArmor(ItemStack aStack, EntityLivingBase aPlayer) {
        if (aPlayer == null || aPlayer.field_70170_p.field_72995_K) return;
        for (EntityEquipmentSlot i : EntityEquipmentSlot.values()) {
            ItemStack tArmor = aPlayer.func_184582_a(i);
                ...
                if (GT_ModHandler.isChargerItem(tArmor) && ElectricItem.manager.getTier(tArmor) >= getTier(aStack)) {
                    ...
                    if (tCharge > 0) {
                        ...
                        if (aPlayer instanceof EntityPlayer) {
                            ...
                            if (tContainer != null) tContainer.func_75142_b();
                        }
                    }
                }
            }
    ...
    */
    public final long getRealCharge(ItemStack aStack) {
        ...
        return tNBT == null ? 0 : tNBT.func_74763_f("GT.ItemCharge");
    }
    ...
    public final boolean setCharge(ItemStack aStack, long aCharge) {
        ...
        if (aCharge > 0) {
            aStack.func_77964_b(getChargedMetaData(aStack));
            tNBT.func_74772_a("GT.ItemCharge", aCharge);
        } else {
            aStack.func_77964_b(getEmptyMetaData(aStack));
        }
        if (tNBT.func_82582_d()) aStack.func_77982_d(null);
        else aStack.func_77982_d(tNBT);
        isItemStackUsable(aStack);
        return true;
    }

    public short getChargedMetaData(ItemStack aStack) {
        return (short) aStack.func_77952_i();
    }
    ...
    public int fill(ItemStack aStack, FluidStack aFluid, boolean doFill) {
        if (aStack == null || aStack.field_77994_a != 1) return 0;
        ...
        Long[] tStats = getFluidContainerStats(aStack);
        if (tStats == null || tStats[0] <= 0 || aFluid == null || aFluid.amount <= 0)
            return 0;
    ...
    public FluidStack drain(ItemStack aStack, int maxDrain, boolean doDrain) {
        if (aStack == null || aStack.field_77994_a != 1) return null;
    ...
    public FluidStack getFluidContent(ItemStack aStack) {
        ...
        NBTTagCompound tNBT = aStack.func_77978_p();
        return tNBT == null ? null : FluidStack.loadFluidStackFromNBT(tNBT.func_74775_l("GT.FluidContent"));
    }

    public void setFluidContent(ItemStack aStack, FluidStack aFluid) {
        NBTTagCompound tNBT = aStack.func_77978_p();
        if (tNBT == null) tNBT = new NBTTagCompound();
        else tNBT.func_82580_o("GT.FluidContent");
        if (aFluid != null && aFluid.amount > 0)
            tNBT.func_74782_a("GT.FluidContent", aFluid.writeToNBT(new NBTTagCompound()));
        if (tNBT.func_82582_d()) aStack.func_77982_d(null);
        else aStack.func_77982_d(tNBT);
        isItemStackUsable(aStack);
    }
