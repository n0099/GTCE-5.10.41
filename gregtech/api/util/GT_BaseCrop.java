...
public class GT_BaseCrop extends CropCard {
    ...
    public ItemStack getGain(ICropTile aCrop) {
        int tDrop;
        if (mSpecialDrops != null && (tDrop = aCrop.getWorldObj().field_73012_v.nextInt((mSpecialDrops.length*2) + 2)) < mSpecialDrops.length && mSpecialDrops[tDrop] != null) {
    ...
    public boolean isBlockBelow(ICropTile aCrop) {
        ...
        for (int i = 1; i < this.getRootsLength(aCrop); i++) {
            IBlockState downState = aCrop.getWorldObj().func_180495_p(aCrop.getPosition().func_177979_c(i));
            int tMetaID = downState.func_177230_c().func_176201_c(downState);
            ItemData tAssotiation = GT_OreDictUnificator.getAssociation(new ItemStack(downState.func_177230_c(), 1, tMetaID));
