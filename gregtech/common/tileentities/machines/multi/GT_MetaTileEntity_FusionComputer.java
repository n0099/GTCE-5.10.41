...
public abstract class GT_MetaTileEntity_FusionComputer extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack) {
        int xCenter = getBaseMetaTileEntity().getXCoord() + EnumFacing.field_82609_l[getBaseMetaTileEntity().getFrontFacing()].func_82601_c() * 5;
        int yCenter = getBaseMetaTileEntity().getYCoord();
        int zCenter = getBaseMetaTileEntity().getZCoord() + EnumFacing.field_82609_l[getBaseMetaTileEntity().getFrontFacing()].func_82599_e() * 5;
                        ...
                        if (mMaxProgresstime > 0 && doRandomMaintenanceDamage()) {
                            ...
                            if (mMaxProgresstime > 0 && ++mProgresstime >= mMaxProgresstime) {
                                ...
                                if (mOutputFluids != null && mOutputFluids.length > 0) {
                                    try {
                                        GT_Mod.instance.achievements.issueAchivementHatchFluid(aBaseMetaTileEntity.getWorldObj().func_72924_a(aBaseMetaTileEntity.getOwnerName()), mOutputFluids[0]);
                                    } catch (Exception e) {
                                    }
                                }
                                this.mEUStore = (int) aBaseMetaTileEntity.getStoredEU();
