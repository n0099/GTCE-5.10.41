...
public abstract class GT_MetaTileEntity_Boiler extends GT_MetaTileEntity_BasicTank {
    ...
    public void saveNBTData(NBTTagCompound aNBT) {
        ...
        if (this.mSteam != null) {
            aNBT.func_74782_a("mSteam", this.mSteam.writeToNBT(new NBTTagCompound()));
        }
    }
    ...
    public void doSound(byte aIndex, double aX, double aY, double aZ) {
        if (aIndex == 1) {
            ...
            for (int l = 0; l < 8; l++) {
                getBaseMetaTileEntity().getWorldObj().func_175688_a(EnumParticleTypes.SMOKE_LARGE, aX - 0.5D + (new XSTR()).nextFloat(), aY, aZ - 0.5D + (new XSTR()).nextFloat(), 0.0D, 0.0D, 0.0D);
            }
        }
    }
