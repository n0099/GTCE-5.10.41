...
public class GT_Cover_Pump extends GT_CoverBehavior {
            ...
            if (tTank2 != null) {
                ...
                if (aCoverVariable % 2 == 0) {
                    ...
                    if (tLiquid != null) {
                        ...
                        tLiquid.amount = tTank2.fill(aSideDirection.func_176734_d(), tLiquid, false);
                        if (tLiquid.amount > 0) {
                            if (((aCoverVariable % 2 == 0) || (aSide != 1)) && ((aCoverVariable % 2 != 0) || (aSide != 0)) && (aTileEntity.getUniversalEnergyCapacity() >= Math.min(1, tLiquid.amount / 10))) {
                                if (aTileEntity.isUniversalEnergyStored(Math.min(1, tLiquid.amount / 10))) {
                                    ...
                                    tTank2.fill(aSideDirection.func_176734_d(), tTank1.drain(aSideDirection, tLiquid.amount, true), true);
                                }
                            } else {
                                tTank2.fill(aSideDirection.func_176734_d(), tTank1.drain(aSideDirection, tLiquid.amount, true), true);
                            }
                    ...
                    FluidStack tLiquid = tTank2.drain(aSideDirection.func_176734_d(), this.mTransferRate, false);
                    if (tLiquid != null) {
                        tLiquid = tLiquid.copy();
                        ...
                        if (tLiquid.amount > 0) {
                            if (((aCoverVariable % 2 == 0) || (aSide != 1)) && ((aCoverVariable % 2 != 0) || (aSide != 0)) && (aTileEntity.getUniversalEnergyCapacity() >= Math.min(1, tLiquid.amount / 10))) {
                                if (aTileEntity.isUniversalEnergyStored(Math.min(1, tLiquid.amount / 10))) {
                                    aTileEntity.decreaseStoredEnergyUnits(Math.min(1, tLiquid.amount / 10), true);
                                    tTank1.fill(aSideDirection, tTank2.drain(aSideDirection.func_176734_d(), tLiquid.amount, true), true);
                                }
                            } else {
                                tTank1.fill(aSideDirection, tTank2.drain(aSideDirection.func_176734_d(), tLiquid.amount, true), true);
                            }
    ...
    public int onCoverScrewdriverclick(byte aSide, int aCoverID, int aCoverVariable, ICoverable aTileEntity, EntityPlayer aPlayer, float aX, float aY, float aZ) {
        aCoverVariable = (aCoverVariable + (aPlayer.func_70093_af() ? -1 : 1)) % 12;
        if (aCoverVariable < 0) {
            aCoverVariable = 11;
        }
