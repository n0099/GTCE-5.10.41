...
public class GT_Block_Metal extends GT_Block_Storage {
    ...
    public GT_Block_Metal(String aName, Materials[] aMats, OrePrefixes aPrefix, Textures.BlockIcons[] aBlockIcons) {
        super(GT_Item_Storage.class, aName, Material.field_151573_f);
        mMats = aMats;
        mPrefix = aPrefix;
        mBlockIcons = aBlockIcons;
        for (int i = 0; i < aMats.length; i++) {
            if (aMats[i].mMetaItemSubID > 0 && aMats[i].mHasParentMod) {
                GT_LanguageManager.addStringLocalization(func_149739_a() + "." + i + ".name", "Block of " + aMats[i].mDefaultLocalName);
